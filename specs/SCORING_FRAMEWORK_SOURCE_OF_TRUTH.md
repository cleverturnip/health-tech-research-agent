...
- **BOUNDARY-VARIANCE DETECTOR + `tier_variance` flag (v1.17; built at assembly, Commit 7).** The gradients
  (bg_fit; growth on the R2 cases) carry LLM run-to-run variance of **±2 on a 1–10 scale** (measured: the
  Commit-4 bg_fit validation reproduced the frozen scores 37/37 within ±2). So a FINAL near a tier boundary
  can flip tier between runs (the season P1/P2 case: bg_fit 7↔5 → FINAL 15↔13). Resolve it HONESTLY rather
  than chase determinism:
  - **DETECTION:** any **floor-PASS** company whose FINAL is within **±2** of a tier boundary (P0/P1 = 18,
    P1/P2 = 15, P2/P3 = 13) is a boundary-straddler. The band **N = ±2 is FIXED, NOT an open dial** — it is
    empirically anchored to the observed ±2 gradient variance, not a guessed number.
  - **RESOLUTION → assign the HIGHER of the two tiers it straddles.** Principled, not a coin-flip: the model
    is a HIGH-RECALL FILTER (§A7) — it surfaces the right ~10–15 and the human deep-researches P0/P1 for
    precision. A false-positive tier is CHEAP (caught + ranked down in human deep-research); a false-negative
    is COSTLY (a real prospect ranked too low, never gets the look). Erring UP on a genuine boundary is the
    error direction the system is built to prefer (same instinct as the Function override: when a company
    COULD belong in the priority set, keep it in and flag it).
  - **FLAG:** attach a `tier_variance` review tag recording the boundary straddled + the variance-sensitive
    inputs (bg_fit; growth for R2 cases). One tag covers both axes — season is doubly variance-sensitive
    (bg_fit + the buried-growth R2 case) and gets a single `tier_variance` flag.
  - **PRECEDENCE (strict order — a company is handled by exactly ONE layer):** **(1) floor rule gates FIRST**
    ...
    P3-by-floor), **(3) boundary-variance detector** on what remains. A floored or human-overridden company is
    NOT double-handled by the detector.
  - **SURFACE (Phase-4):** a boundary-straddler is floor-PASS, so it gets a CARD (not just a summary
    one-liner); the card shows the `tier_variance` flag and the human confirms or moves the tier.
  - **R1 (Commit 8) inherits a CLEAN target:** `season` is DEFINED as **P1 with a `tier_variance` flag**, so
    the R1 criterion is "reproduce P0=4 / P1=6 / P2=6 / P3=38 with season at P1, flagged" — deterministic and
    reproducible. The variance stops being a parity threat and becomes a documented expected-flag.
