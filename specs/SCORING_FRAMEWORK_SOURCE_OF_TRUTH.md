...
- **RESET (ZOE-validated; SHARPENED v1.5):** fires on a qualifying event — genuine leadership change,
  ...
  - **Confidence bar — "unclear" does not fire (noom):** a reset fires ONLY on a CLEARLY qualifying
    ...
    - **STATED-PURPOSE BOUNDARY — RATIFIED for R1; sharper version DEFERRED post-R1.** The emitter draws the
      growth-support `unclear` vs build-mandate `yes` line on the evidence's STATED PURPOSE: "to support /
      accelerate existing growth/expansion" → `unclear` (noom CMO, sword CRO); "builds a new operating
      function / leads a turnaround" → can be `yes` (grow's first-ever CFO "scale finance with operating
      rigor"). This reproduces the spike's `_GROWTH_SUPPORT` regex behavior, so it is **RATIFIED for R1**
      (faithful parity — re-drawing it could move a company and break the calibration). A sharper
      **structural-role-over-stated-wording** rule (e.g. "a first-ever CFO is a structural build regardless
      of the verb") is a **post-R1 consideration, NOT folded into the port** — same deferral discipline as
      the §B6 unknown-stage improvement (decided with eyes open after R1, never swapped in silently).
