...
- **REPRODUCIBLE SCORING via PERSISTED READS (caching) + `tier_review` proximity flag (v1.22 — SUPERSEDES
  ...
  - **`tier_review` PROXIMITY FLAG (margin = ±1, floor-PASS only):** a company whose stable FINAL sits
    **within 1** of a tier boundary — i.e. `threshold_tier(FINAL−1) != threshold_tier(FINAL)` OR
    ...
    threshold-driven.)
  - **`floored_bg_near_threshold` — the FLOORED-BUT-CLOSE review flag (v1.23; the piece that makes the P3
    floor trustworthy under caching).** Katelynd deep-dives every P0/P1 by hand, so top-bucket errors
    self-correct; the EXPENSIVE error is a real prospect frozen-LOW in the un-reviewed P3 pile (`grow`: its
    bg froze at 4 → floored, but it is a real P0 candidate — it only surfaced because she happened to know
    it). So a company floored ONLY by a low bg whose FROZEN bg could plausibly have rolled to passing is
    surfaced into the bounded `review_set`. ALL must hold: (a) floored on **bg** specifically (NOT gate-
    floored maturity/B2B — those are deterministic, a re-read wouldn't change them; NOT pmf); (b) would PASS
    if bg cleared (pmf > 4); (c) bg is a real low read in **{3, 4}** — the passing line is bg > 4 (=5) and
    `BG_FLOOR_WOBBLE = 2` is anchored to the OBSERVED pre-caching bg wobble (typical ±1, `season` ±2, `grow`
    ±3 — its 4→8 swing), so a frozen bg within 2 of the line could have wobbled to passing. A bg=8 could not
    have; a bg=1 could not have; an ABSENT bg is a READ-FAILURE (a different flag), not this. **Action = a
    REVIEW flag → consider a deliberate, logged `--refresh <company>` (re-take that one company's bg); NEVER
    an auto-un-floor, NEVER a revert to multi-run.** A refresh that comes back ALSO low confirms the floor.
    This keeps `review_set` bounded (~this-roster: `grow`/`cylinder`/`oula`) while ensuring the floor is not
    hiding a `grow`; everything else in P3 floored for a stable, non-wobbling reason (maturity / B2B /
    genuinely-low pmf) that can be trusted un-reviewed.
