...
- **BOUNDARY-VARIANCE DETECTOR + `tier_variance` flag (v1.20 — RUN-TO-RUN STABILITY check; built at
  ...
  - **STABLE (same tier all 5 runs) → that tier stands; NO flag. UNSTABLE (the tier differs on ANY of the 5
    runs) → `tier_variance` straddler → assign the HIGHEST tier observed across the 5 runs + the
    `tier_variance` flag.** High-recall (§A7): any cross treats the company as the higher tier — a
    false-positive tier is CHEAP (caught + ranked down in human deep-research), a false-negative is COSTLY (a
    real prospect never gets the look); erring UP is the error direction the system prefers (same instinct as
    the Function override: when a company COULD belong in the priority set, keep it in and flag it).
  - **N = 5 is an HONEST DIAL (FIXED for R1):** more runs = more confidence a wobble is caught — it is the
    actual measurement of instability, NOT a proxy for it.
  ...
  - **R1 (Commit 8) IS the stability machinery:** R1 re-validation runs each floor-PASS, non-overridden
    company **5×** — so the target is "reproduce P0=4 / P1=6 / P2=6 / P3=38 with **`season` as the one
    company whose TIER MOVES across the 5 runs → assigned P1, flagged**, and the six FINAL-14 companies
    STABLE at P2 (no flag)." The stability check and R1 are the same measurement; the variance becomes a
    documented expected-flag, not a parity threat.
