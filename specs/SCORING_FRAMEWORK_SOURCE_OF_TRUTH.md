...
- **BOUNDARY-VARIANCE DETECTOR + `tier_variance` flag (v1.17; built at assembly, Commit 7).** The gradients
  ...
  - **DETECTION:** any **floor-PASS company NOT resolved by a human override** whose FINAL is within **±2**
    of a tier boundary (P0/P1 = 18, P1/P2 = 15, P2/P3 = 13) is a boundary-straddler. The band **N = ±2 is
    FIXED, NOT an open dial** — it is empirically anchored to the observed ±2 gradient variance, not a guessed
    number. (A human-overridden company is TERMINAL per PRECEDENCE below — its tier is the override's final
    word, so it is never a boundary-straddler even when its FINAL sits within ±2; the detector does not
    re-touch it. Stated here at the detection point, not only in PRECEDENCE, so it cannot be misread in
    isolation — v1.19.)
