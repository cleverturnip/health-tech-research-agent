...
- **GROWTH READ — BAND CLASSIFICATION (v1.24; SUPERSEDES Scale-B-interpolation-FOR-GROWTH + the entire v1.23
  ...
  - **CLASSIFICATION, NOT DERIVATION (the structural win):** the extractor BANDS; it NEVER derives a rate by
    combining figures. Single-source is fine (a rate stated by the company OR one analyst is a valid banding
    signal); it MUST NEVER combine two DIFFERENT sources. Because it bands rather than derives, the whole v1.23
    same-source subsystem (report-figures schema, `derive_growth_from_figures`, `GROWTH_SOURCE_ALIASES`) is
    SUPERSEDED and REMOVED — no derivation = no cross-source bug = no yo-yo. The extractor RECORDS the evidence
    it banded on (the rate/trajectory + its source) for the floor_reason / review trail.
...
- **GROWTH-EXTRACTION SCOPE (v1.6 — zero-baseline path SUPERSEDED by the band read; FENCE KEPT).**
  ...
  magnitude path in the growth read. (2) **DERIVED / third-party single figures** are still real evidence and
  BAND (a credible single-source estimate is a valid banding signal) — but they are never COMBINED into a
  rate. The **§B6.1 FENCE is KEPT (unchanged, load-bearing under the band read too):** growth_score is
