...
- **BOUNDARY-VARIANCE DETECTOR + `tier_variance` flag (v1.20 — RUN-TO-RUN STABILITY check; built at
  assembly, Commit 7).** The gradients (bg_fit; growth on the R2 cases) carry LLM run-to-run variance
  (measured: the Commit-4 bg_fit validation reproduced the frozen scores 37/37 within ±2), so a company's
  SCORE can MOVE between runs — and a company near a tier line can land in different tiers on different runs
  (the season case: bg_fit 7↔5 → FINAL 15↔13 → P1↔P2). The detector fires on that **INSTABILITY** — a
  company whose TIER moves across runs — **NOT on mere proximity to a boundary.** Resolve it HONESTLY rather
  than chase determinism:
  - **MECHANISM (N = 5):** for each **floor-PASS company NOT resolved by a human override**, score it
    **N = 5 independent times**. ONLY the LLM-variable inputs re-run (bg_fit; growth on R2 cases); the
    deterministic parts (strain, floor, thresholds, stage) are identical every run BY CONSTRUCTION — do NOT
    re-roll them as if stochastic. Observe the resulting TIER on each of the 5 runs.
  - **STABLE (same tier all 5 runs) → that tier stands; NO flag. UNSTABLE (the tier differs on ANY of the 5
    runs) → `tier_variance` straddler → assign the MODAL tier (the MOST FREQUENT tier across the N runs; a
    TIE resolves toward the HIGHER tier) + the `tier_variance` flag** (v1.21 — was "highest observed"; see
    AUDIT). The flag STILL fires on ANY variance (the high-recall SURFACING is preserved — every straddler
    goes to the human), but the ASSIGNED tier is the company's TYPICAL run, not its best-case run. The
    tie-break-toward-higher keeps a residual err-UP (§A7) at a genuine 50/50 split, without letting a single
    lucky run set the tier.
  - **N = 5 is an HONEST DIAL (FIXED for R1):** more runs = more confidence a wobble is caught, and a more
    stable MODE — it is the actual measurement of instability, NOT a proxy for it.
  - **FLAG:** `tier_variance` records the boundary the tier moved across + the variance-sensitive inputs
    (bg_fit; growth on R2 cases). One tag covers both axes — season is doubly variance-sensitive (bg_fit +
    the buried-growth R2 case) → a single `tier_variance` flag.
  - **PRECEDENCE (strict order — a company is handled by exactly ONE layer):** **(1) floor rule gates FIRST**
    (floor-FAIL → P3 regardless), **(2) human overrides** (Rule 6 — Function P1-override, Angle/Oula
    P3-by-floor), **(3) stability detector** — ONLY floor-PASS, non-overridden companies are scored N×. A
    floored or human-overridden company is NOT scored for stability and NOT flagged.
  - **AUDIT TRAIL (record the WHY so it is never "simplified" back to proximity):** v1.17 encoded this intent
    as boundary-PROXIMITY ("any floor-PASS company within ±2 of a boundary → bump higher"). A deterministic
    pre-Commit-7 check found that COLLAPSES P2: all six FINAL-14 companies (affect, equip, familywell, fay,
    foodsmart, jasper) sit within ±2 of the 15 boundary → all would bump → P2=0, contradicting the R1 target
    AND v1.17's own "only season changes" claim. The intent was always run-to-run INSTABILITY (a score that
    MOVES), not proximity (a stable score that happens to sit near a line) — the "±2" was score-movement,
    mis-attached to boundary-distance (an intent-to-action mis-encoding; see `COLLABORATION_CONTEXT.md`
    "Intent-to-action confirmation"). v1.20 restores the intent as the N-run stability check.
  - **AUDIT — v1.21 MODE (the R1 live run falsified "highest observed"):** v1.20 chose "highest tier
    observed" (err-UP) on the ASSUMPTION of FEW straddlers (only `season`). The live R1 run FALSIFIED that:
    broad live bg_fit/growth variance made **9** companies straddle, and "highest observed" applied broadly
    INVERTED the distribution (P0=**7** vs 4; `affect`/`season` were P3 in 4 of 5 runs but resolved P2 off a
    single outlier; `equip`/`hinge` P1 in 3/5 resolved P0). "Highest observed" makes the tier the LUCKIEST
    run — indefensible under broad variance. MODE reports the CENTRAL tendency; the flag still surfaces every
    straddler. (Intent-to-action: err-UP was the right instinct for RARE straddlers, the wrong rule for
    COMMON ones — the number of straddlers is the input the v1.20 rule silently assumed.)
  - **SURFACE (Phase-4):** a flagged straddler is floor-PASS → it gets a CARD; the card shows the
    `tier_variance` flag (the boundary its tier moved across) and the human confirms or moves the tier.
  - **R1 (Commit 8) IS the stability machinery — the target is an OUTPUT, re-validated by name, NOT forced:**
    R1 re-validation runs each floor-PASS, non-overridden company **5×**. The spike's `P0=4/P1=6/P2=6/P3=38`
    was a FROZEN-score ESTIMATE; the live hardened system produces its own distribution for REAL reasons (the
    six frozen-at-FINAL=14 spike-P2 companies were coin-flips and scatter under live scoring), and the
    RE-VALIDATED target is whatever the correct system produces, ratified company-by-company. Forcing the
    live output back to 4/6/6/38 (via a shim, a threshold nudge, or a silent adjustment) is a FAILED R1; a
    distribution understood by name is a PASSED one.
