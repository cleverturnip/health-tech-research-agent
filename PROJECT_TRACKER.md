...
> **Why this is the current milestone.** Slices 1 (PR #29) and 2 (PR #31) are **done** — the research runner and the structured maturity/commercial evidence now live in the package; **Slice 3 (reset)** is next. The dependency cascades: **Slices 3–4** (reset → real capability-fit) → **full data regeneration** (run-once) → **Commit 5** (candidate → `final_priority_level` authority) → **Commit 6** (master remediation) → **calibration**.
