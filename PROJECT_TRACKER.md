...
> **Why this is the current milestone.** Slices 1 (PR #29), 2 (PR #31), 3 (PR #33), and 3.5 (PR #34) are **done** — the research runner, the structured maturity/commercial evidence, and the researched (now multi-event) reset signal live in the package. Next is **Slice 3.7 (search-layer redesign)** — fix the operator/organizational coverage gap (reset + capability-fit had no search behind them) and the stale one-bullet evidence-quality constraint — then **Slice 4 (capability-fit)**, the last build before regeneration. The dependency cascades: **Slices 3.7 → 4** → **full data regeneration** (run-once) → **Commit 5** (candidate → `final_priority_level` authority) → **Commit 6** (master remediation) → **calibration**.
...
- [ ] **Slice 3.7 — Search-layer redesign (coverage + evidence quality)** — add two operator/organizational searches and re-budget the four existing ones. `search_org_events` (recency-bounded ~12–18 mo, multi-item list) feeds reset; `search_operating_characteristics` (product-engagement structure + operational-strain lenses) feeds capability-fit A1/A2/A3. Re-budget commercial to gather rich, provenance-tagged, trend-aware evidence (q1–q4 stay in the synthesis); add `founding_year` to funding; retire the stale one-bullet constraint; lift token ceilings (~600–900) on the high-demand searches. Fixes the reset-coverage finding (above) and the evidence-quality gap from the two read-only audits. Build + Colab-verify BEFORE Slice 4 (capability-fit consumes `search_operating_characteristics`) and before regeneration. Spec: `specs/slice3_7_search_layer_redesign_spec.md`
- [ ] **Slice 4 — Real capability-fit (3-attribute A1/A2/A3)** — replaces the interim `role_fit` bridge, flips the framework off "V4.2-interim", and unblocks Commit 5. Consumes Slice 3.7's `search_operating_characteristics`; built against the **reframed A1 (product-engagement structure) + A2 (operational strain)** definitions (Slice 3.7's spec is the source of truth; `specs/slice4_capability_fit_spec.md` points to it). (Same item is cross-referenced under Phase 3 → Candidate Priority Engine.) Spec: `specs/slice4_capability_fit_spec.md`

> After Slices 3.7 + 4 land (Slices 1–3.5 done), the downstream held chain runs **in order** — full data regeneration (run-once) → Commit 5 → Commit 6 → calibration — tracked under Phase 3 → Candidate Priority Engine. Operational do-before-refresh reminders live in `specs/phase2_refresh_runbook.md`.
