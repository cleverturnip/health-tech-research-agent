...
> **Why this is the current milestone.** Slices 1 (PR #29), 2 (PR #31), 3 (PR #33), 3.5 (PR #34), and 3.7 (PR #36) are **done** — the research runner, the structured maturity/commercial evidence, the researched (now multi-event) reset signal, and the redesigned six-search evidence layer (two operator/organizational searches + re-budgeted existing ones) live in the package. Next is **Slice 4 (capability-fit)** — the last build before regeneration; it consumes Slice 3.7's `search_operating_characteristics`. The dependency cascades: **Slice 4** → **full data regeneration** (run-once) → **Commit 5** (candidate → `final_priority_level` authority) → **Commit 6** (master remediation) → **calibration**.
...
- [x] **Slice 3.5 — Multi-event reset (per-event opening)** — `reset_evidence.reset_events` becomes a LIST; each event's opening is assessed on its own terms, so a pivot's "no" can't bury a restructuring's "yes" (the ZOE finding). Fires iff ANY non-never-fire event has opening=yes; empty list → False; an unrecognized event type doesn't fire and sets `reset_needs_review`. (PR #34; live-verified in Colab — Noom strategic-pivot correctly does NOT fire; list-model wiring + master read-back confirmed.) Spec: `specs/slice3_addendum_multi_event_reset.md`
  - *Live finding (research-coverage, not a 3.5 defect): ZOE returned an EMPTY events list — neither its pivot nor its restructuring surfaced — because reset is synthesized from the four market/financial searches (the fit brief runs web-search OFF) and no search targets leadership/restructuring events. The multi-event FIRE is proven by the offline red→green test; surfacing it on live data is **fixed by Slice 3.7** (`search_org_events`).*
- [x] **Slice 3.7 — Search-layer redesign (coverage + evidence quality)** — added two operator/organizational searches and re-budgeted the four existing ones. `search_org_events` (recency-bounded ~12–18 mo, multi-item list) feeds reset; `search_operating_characteristics` (product-engagement structure + operational-strain lenses) feeds capability-fit A1/A2/A3. Commercial now gathers provenance-tagged + trend-aware evidence (q1–q4 stay in the synthesis); funding gained `founding_year`; the stale one-bullet constraint is retired; token ceilings lifted (~600–900) on the high-demand searches. Synthesis nudges added (reset reads the org-events section + carries the opening through; commercial reads SOURCE TYPE/TREND) — no capability scoring yet (Slice 4). Notebook mirror: nine-column research schema + STEP 26 six-section rescore + embedded checklist. Fixes the reset-coverage finding (above) and the evidence-quality gap from the two read-only audits. (PR #36; verified **offline** — `compile()` after stripping magics + 214-test suite. **LIVE Colab verification still pending** — the embedded checklist: ZOE multi-event reset surfaces + fires, both new findings populate, STEP 26 uses the new sections; runs at the regeneration / next Colab run, per the runbook.) Spec: `specs/slice3_7_search_layer_redesign_spec.md`
- [ ] **Slice 4 — Real capability-fit (3-attribute A1/A2/A3)** — **now unblocked (Slice 3.7 merged).** Replaces the interim `role_fit` bridge, flips the framework off "V4.2-interim", and unblocks Commit 5. Consumes Slice 3.7's `search_operating_characteristics`; built against the **reframed A1 (product-engagement structure) + A2 (operational strain)** definitions (Slice 3.7's spec is the source of truth; `specs/slice4_capability_fit_spec.md` points to it). (Same item is cross-referenced under Phase 3 → Candidate Priority Engine.) Spec: `specs/slice4_capability_fit_spec.md`

> After Slice 4 lands (Slices 1–3.7 done), the downstream held chain runs **in order** — full data regeneration (run-once) → Commit 5 → Commit 6 → calibration — tracked under Phase 3 → Candidate Priority Engine. Operational do-before-refresh reminders live in `specs/phase2_refresh_runbook.md`.
...
### Candidate Priority Engine (deterministic; shipped — framework real **V4.2** as of PR #38; INERT until Commit 5)
...
- [x] **Real LLM-scored capability-fit (3-attribute A1/A2/A3)** — DONE (Slice 4, PR #38). Replaced the interim `role_fit` bridge; framework is now real V4.2.

Downstream chain (runs **in order**):
- [x] (**RUN-ONCE** — DONE, 2026-06-24) **Full data regeneration** — clean-slate re-research on the hardened pipeline → the canonical **55-company V4.2 master** (every row `New batch - needs review`; `final_priority_level/_code/_rank` deliberately blank for Commit 5; `videahealth` deliberately absent). Integrity gaps from the audits cleared. See `specs/regen_execution_runsheet.md`.
- [x] (**DONE, 2026-06-24** — PR #41) **Field-landing remediation** — a post-regen audit found two LLM-JSON clusters present-but-blank on all 55 master rows though populated 55/55 in the saved checkpoint: **role/timing** (`stage_timing_fit`, `likely_agency_level`, `why_now_or_why_not`) and **taxonomy-LLM** (`primary_market_segment`, the four tag columns, `taxonomy_assignment_method/_basis`). Root cause: the inline STEP 10 summary build dropped both clusters from the summary→master landing (and the regen read-back checked column *presence*, not *population*). Re-landed — no re-research, no master hand-edit — by the standalone `reland.py`: blank-only fill, 1:1 completeness guard, per-field read-back vs the checkpoint's own counts, backup + rollback. Colab-verified (role/timing 55/55, `subsegment_tags` 54, and `final_priority_level` + `primary_market_segment_code` stayed blank). `primary_market_segment_code` deferred to STEP 14 (validated code = classifier-owned, Rule 7); the 4 role/timing siblings + 5 deterministic-taxonomy fields deferred to the dashboard milestone.
- [ ] (**NEXT** — unblocked: real capability-fit + trusted regenerated data + engine inputs now landed) **Commit 5** — make candidate priority authoritative for `final_priority_level` unless a row passed the human review gate; fix the false "Human Reviewed" labeling (gate-passage is the discriminator, not value change) and the sticky `reviewed_priority_level` auto-seed. Locked design: `specs/candidate_priority_reference_spec.md` §10.
...
- [ ] (deferred — until the back-half migration completes: Slices 2–4 + regeneration + Commit 5) **Post-migration cleanup pass on `colab_workflow.py`** — prune superseded / dead notebook code: old steps no longer used, dead helper functions, abandoned experiments, and especially the **leftover engine generations** (the multiple `compute_candidate_priority` versions, and double-defined functions like `step26_infer_maturity_and_cap`) not already retired by the slice work. Also retire-or-redirect the residual **reset-flavored text-scans** the slices left in place: `_rt_has_high_agency_exception` (STEP 10 timing-cap) and STEP 26's `high_agency_terms` — either retire them or point them at the researched reset opening (Slice 3). *Why the timing:* doing this BEFORE the migration finishes would mean auditing code the migration is about to delete anyway, and risks removing something a pending slice still depends on; after the back half is complete and the suite is comprehensive, the dead code is obviously dead and the cleanup can be aggressive and low-risk. The leftover engine generations are an active **"which-version-ran-last" hazard**, not just clutter — so they are the priority of that pass.
  - [ ] **Stale inline `search_funding` in the notebook (~line 14600)** — a pre-migration 300-token "1 bullet, under 100 words" version (with its own inline `call_openai`), used by the old-flow "Run missing companies" loop (~line 16318). The live/regen path delegates to the package (`run_research_batch` → `research_runner.search_funding`, 700-token FACT LIST), so this is **superseded** — but it's a **name-shadowing hazard**: the bare global `search_funding` resolves to whichever cell ran last, so running the old-flow cell silently swaps in the stale 300-token version. Prune-candidate, same "which-version-ran-last" hazard class. Found during the 2c funding-variance reconciliation.
